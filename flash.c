...
{
	if(ssd->nvram_seg[block].next_avail_time < ssd->current_time)
	{
		ssd->nvram_seg[block].next_avail_time = ssd->current_time + need_time;
	}
	else
	{
		ssd->nvram_seg[block].next_avail_time += need_time;
	}
